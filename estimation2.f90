...
function log_likelihood2(params_MLE)
    ...
    print*,'Non-constrained pr sample',pr_non_zombie_an(:,1)
...
!    log_likelihood2=-log_likelihood2
    ...
    do n_l=1,2;do a_l=1,types_a
        pr_d_a_n_av(a_l,n_l)=sum(pr_d_a_n(a_l,n_l,:)*pr_non_zombie_anv(n_l,a_l,:)*shares_n_a_v(a_l,n_l,:)) !pr_d_a_n(1,1,1)*pr_non_zombie_anv(1,1,1)
    end do;end do
    ...
    log_likelihood2=sum((pr_d_a_n_av-moment_own_nxa_data)**2.0d0)+(pr_n_av(1)-pr_little_n_data(1))**2.0d0 + &
                    sum((pr_N_n_av(1,:)-pr_N_n_data(1,:))**2.0d0) !+ sum((pr_d_w_av*pr_non_zombie_w-moment_w_data)**2.0d0)
