subroutine compute_eq_F_CCP(params,F,CCP,V_fct,V_social,n_initial,v_l,mean_N,social_output,private_output,joint_pr,pr_d_a_n,pr_N_n,pr_na)
    ...
    do a_l=1,types_a;do u_l=1,unobs_types
        call expected_productivity((/params(2),params(1),params(7)/),area(a_l),Ef_v(:,:,:,a_l,v_l,u_l),v_l,u_l) !Ef_v(1,:,1,1,1,:)
...
!    F=1.0d0
    ...
    n_start=n_initial
    1 n_initial=1
    call generate_beliefs(CCP,V_fct,V_social,Ef_v(:,:,:,:,v_l,:),n_initial,F,v_l,iterations,mean_N,social_output,private_output,joint_pr,pr_d_a_n,pr_N_n,pr_na) 
    ...
    do P_l=1,P_max; do a_l=1,types_a; do u_l=1,unobs_types
        call value_fct_it(Ef_v(1:2*P_l-1,:,P_l,a_l,v_l,u_l)& !Ef_v(1:2*P_l-1,2,P_l,a_l,v_l,:)
                            ...
                            ,P_l &
                            ,CCP(1:2*P_l-1,:,P_l,a_l,u_l),v_l,u_l & !CCP2(1,:,1,4,:)
                            ,V_fct(1:2*P_l-1,:,P_l,a_l,u_l)) !V_fct(1,:,1,4,:)
        social=0
